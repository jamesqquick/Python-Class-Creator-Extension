...
function activate(context) {
	...
	console.log('Congratulations, your extension "python-class-generator" is now active!');

	// The command has been defined in the package.json file
	// Now provide the implementation of the command with  registerCommand
	// The commandId parameter must match the command field in package.json
	let disposable = vscode.commands.registerCommand('extension.createPyClass', async function () {
		...
		const className = await vscode.window.showInputBox({ prompt: "Class Name?" });
		vscode.window.showInformationMessage(`Hello ${className}`);
		let count = 1;
		let property = await vscode.window.showInputBox({ prompt: `Property #${count}? ('done' when finished)` });
		const properties = []
		while (property != 'done') {
			properties.push(property)
			count++;
			property = await vscode.window.showInputBox({ prompt: `Property #${count}? ('done' when finished)` });
		}

		const classDefinition = `class ${className}:`
		const constructorDefinition = `def __init__(${properties.join(', ')}):`
